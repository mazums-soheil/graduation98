(() => {
  ...
  const criticalManifest = [
    ...
    'assets/hero-poster-1600.webp',
    'assets/campus-aerial-1600.webp',
    'assets/campus-drive-1600.webp'
  ];
  ...
  function setLoader(p, status = '') {
    ...
    if (loaderPercent) loaderPercent.textContent = `${v.toLocaleString('fa-IR')}٪`;
  ...
  async function readResponseBlobWithProgress(response, onProgress) {
    ...
    for (;;) {
      ...
      if (total > 0) onProgress?.(clamp(loaded / total));
      else onProgress?.(Math.min(.94, .08 + Math.log2(1 + loaded / 65536) / 12));
    }
    onProgress?.(1);
    return new Blob(chunks, { type: response.headers.get('content-type') || 'video/mp4' });
  }

  async function preloadHeroVideoBlob(onProgress) {
    if (preloadedVideoURL) {
      onProgress?.(1);
      return preloadedVideoURL;
    }
    if (heroBlobFetchPromise) return heroBlobFetchPromise;
    heroBlobFetchPromise = (async () => {
      try {
        onProgress?.(.02);
        const response = await fetch(heroVideoSource(), {
          cache: 'force-cache',
    ...
    const result = await heroBlobFetchPromise;
    if (!result) heroBlobFetchPromise = null;
    return result;
  }
  ...
  async function warmSectionMedia(index, { guarded = false } = {}) {
    ...
    const job = (async () => {
      let results = await Promise.all(urls.map((url) => preloadImage(url, { priority: guarded ? 'high' : 'auto' })));
      if (results.some((ok) => !ok)) {
        await new Promise((resolve) => setTimeout(resolve, 280));
        results = await Promise.all(urls.map((url) => preloadImage(url, { priority: 'high' })));
      }
  ...
  function updateHeroUI(p) {
    ...
    if (shine) shine.style.transform = `translateX(${(-130 + p * 260).toFixed(1)}%)`;
  }
  ...
  async function init() {
    ...
    const boot = { visuals: 0, video: 0, fonts: 0 };
    const paintBoot = (status) => {
      const progress = .04 + boot.visuals * .34 + boot.video * .48 + boot.fonts * .10;
    ...
    heroBlobWarmPromise = preloadHeroVideoBlob((p) => {
      boot.video = p;
      paintBoot(p < 1 ? 'در حال دریافت ویدیوی آغازین…' : 'ویدیوی آغازین دانلود شد');
    });
    ...
    const [visualsResult, videoResult] = await Promise.allSettled([visualsTask, heroBlobWarmPromise, fontsTask]).then((results) => [results[0], results[1]]);
    const visualsOK = visualsResult.status === 'fulfilled' && visualsResult.value !== false;
    const videoBlobOK = videoResult.status === 'fulfilled' && !!videoResult.value;


    setLoader(.965, 'در حال آماده‌سازی نخستین فریم…');
    let videoReady = false;
    if (videoBlobOK && preloadedVideoURL) {
      heroVideoLoadPromise = null;
      heroVideoFailed = false;
      videoReady = !!(await withTimeout(bindVideoSource({ preferBlob: true }), 7000));
    }
    if (!videoReady) videoReady = await ensureHeroVideoReadyForStory(12000).catch(() => false);

    if (videoReady) sectionMediaReady.add(0);
    if (visualsOK) sectionMediaReady.add(1);

    // Keep a short minimum duration only to avoid a flash on warm-cache revisits;
    // readiness itself is driven by real media completion above.
